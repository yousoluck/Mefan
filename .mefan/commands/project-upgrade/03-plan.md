...
## 3. 强制规则
- `.mefan/knowledge/global/conflict-resolution.md`
- `.mefan/knowledge/scenario-upgrade/reuse-before-build.md`（检查是否有任务可复用现有代码）
...
2. 对每个任务运行的模块，执行冲突检测：
   - 若任务涉及模块与其他活跃任务产生**核心冲突**，按 `.mefan/knowledge/global/conflict-resolution.md` 决策树处理：
     a. 串行化（优先）
     b. 分模块（若可拆分）
     c. 人类裁决（生成《冲突裁决申请书》）
