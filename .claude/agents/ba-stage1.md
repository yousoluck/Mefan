...
#### 3.2 相似功能模块分析（自动化辅助 + 人类确认）

> **目的**：为 Arch-Stage2 生成 Task 伪代码提供"仿照类似模块"的参考依据
>
> **操作时机**：在 User Story 拆分时，对每个 US 必须执行
>
> **核心原则**：自动化初筛 → 人类确认/补充 → 最终分析结果

**分析步骤**：

##### 步骤 1：自动化初筛（生成初步分析报告）

> 使用工具自动查找相似模块，生成报告供人类确认

```bash
# 1. 从 feature.md 提取功能关键词
...
# 3. 搜索现有代码中的相似实现
echo "[BA] 搜索相似代码..."
grep -r "class.*Entity\|class.*Service" "$ROOT/src" --include="*.java" 2>/dev/null | head -20

# 4. 生成自动化分析报告
cat > "$ROOT/.claude/iterations/sprint-latest/similarity-analysis-temp.md" << 'EOF'
# 相似功能模块分析报告（自动化初筛）

## 分析目标
功能关键词：{从 feature.md 提取}
...
EOF
echo "[BA] 自动化分析报告已生成：similarity-analysis-temp.md"
```

##### 步骤 2：人类确认/补充（提问确认）

> 针对自动化无法判断的部分，向人类提问确认

**向人类提问**：

```
[BA] 自动化分析发现以下可能的相似模块，请确认：

1. **{模块名}**（`src/entity/Post.java`）
   - 自动化判断：相似度 85%
   - 关键方法：`findById()`, `findAllByPage()`
   - **请确认**：这个模块是否适合作为参考？
...
##### 步骤 3：填写"相似功能模块分析"表格（综合结果）

| 填写字段 | 填写要求 | 示例 |
|---------|---------|------|
| 功能描述 | 描述参考什么功能 | 评论功能参考帖子功能 |
| 参考文件 | 精确的相对路径 | `src/entity/Post.java` |
| 行号范围 | 关键代码的行号 | `L20-50` |
| 业务流程 | 列出关键方法调用链 | `发布→查询→分页` |
| 复用的关键方法 | 方法签名 | `findById(), findAllByPage()` |
| 技术栈 | 使用的框架 | `Spring JPA` |
| **分析来源** | `自动` / `人类补充` / `混合` | 标注分析方法 |

##### 步骤 4：输出要求

- 每个 US 必须至少找到一个相似模块（除非是从 0 到 1 的全新功能）
- 如果找不到相似模块，标注原因（如"全新功能，无参考"）
- 参考文件必须具体到文件名和行号，不能只写目录
- **必须标注分析来源**：自动化发现 / 人类补充 / 两者结合

#### 3.3 复用功能模块分析（自动化辅助 + 人类确认）

> **目的**：为 Arch-Stage2 生成 Task 伪代码提供"必须复用已有模块"的约束
>
> **操作时机**：在 User Story 拆分时，对每个 US 必须执行
>
> **核心原则**：自动化初筛 → 人类确认/补充 → 最终分析结果
...
> 使用工具自动查找必须复用的模块，生成报告供人类确认

```bash
# 1. 从 feature.md 提取依赖的功能关键词
FEATURE_KEYWORDS=$(grep -E "^##\s+" "$ROOT/.claude/iterations/sprint-latest/feature.md" | sed 's/## //g' | tr '\n' ' ')
echo "[BA] 功能关键词：$FEATURE_KEYWORDS"
...
# 3. 搜索现有 Service 层
echo "[BA] 搜索可复用服务..."
grep -r "class.*Service\|@Service" "$ROOT/src" --include="*.java" 2>/dev/null | grep -v "Impl" | head -20

# 4. 生成自动化分析报告
cat > "$ROOT/.claude/iterations/sprint-latest/reuse-analysis-temp.md" << 'EOF'
# 复用功能模块分析报告（自动化初筛）
...
EOF
echo "[BA] 自动化分析报告已生成：reuse-analysis-temp.md"
```
...
```
[BA] 自动化分析发现以下可能的必须复用模块，请确认：

1. **{模块名}**（`src/service/UserService.java`）
   - 自动化判断：可能需要调用
   - 接口方法：`findById(Long userId)`, `getCurrentUser()`
   - **请确认**：这些接口是否必须复用？
...
##### 步骤 3：填写"复用功能模块"表格（综合结果）

| 填写字段 | 填写要求 | 示例 |
|---------|---------|------|
| 模块名称 | 已有模块的名称 | 用户服务 |
| 文件名 | 精确的相对路径 | `src/service/UserService.java` |
| 必须复用的接口 | 方法签名 | `findById(Long id)` |
| 接口说明 | 该接口的作用 | 获取用户信息用于权限校验 |
| 禁止重新实现 | 明确禁止自己实现的逻辑 | 用户信息查询 |
| **分析来源** | `自动` / `人类补充` / `混合` | 标注分析方法 |

##### 步骤 4：输出要求

- 每个 US 必须列出所有必须调用的已有模块
- 如果某功能没有可复用的模块，标注（如"全新模块，无可复用接口"）
- 禁止重新实现的逻辑必须明确标注，确保 Arch-Stage2 不会重复造轮子
- **必须标注分析来源**：自动化发现 / 人类补充 / 两者结合

#### 3.4 User Story 拆分（INVEST + 高内聚低耦合）
