...
1. **识别冲突**：读取 `iterations/sprint-status.md` 中已记录的任务模块清单，与本需求的“触达模块”交叉比对。
   - 若任一模块已在看板其他任务中存在，记录为冲突。
...
4. **升级记录**：若产生核心冲突，PM 在 `iterations/session-status.md` 的“异常记录”段追加：
