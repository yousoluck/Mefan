...
## 1. 角色激活
- **主导 Agent**：QA 工程师 (`.claude/agents/qa.md`)，负责测试执行、缺陷记录、人工测试指南。
- **修复 Agent**：开发者 (`.claude/agents/developer.md`)，负责修复被驳回的缺陷。
- **终审 Agent**：守护者 (`.claude/agents/guardian.md`)，执行最终门禁裁定。
- **决策 Agent**：项目经理 (`.claude/agents/pm.md`)，处理 P0 缺陷和进度异常。
...
1. 按 `.claude/skills/write-manual-test-guide.md` 严格按照 `.mefan/templates/manual-test-guide-template.md` 生成 `.mefan/iterations/{sprint-name}/test-results/manual-test-guide.md`。
2. 内容必须包含：
   - 实现的功能清单及对应文件路径
