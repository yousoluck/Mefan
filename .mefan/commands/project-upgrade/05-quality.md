...
- `skills/write-manual-test-guide.md`
- `skills/bug-triage-classification.md`

## 3. 强制规则
- `knowledge/global/quality-gates.md`
...
1. **回归测试**：
   ...
   - 记录通过/失败结果到 `test-results/regression-YYYY-MM-DD.log`。
   - 若回归测试发现失败，立即记录到 `bug-log/auto-YYYY-MM-DD.md`（每条失败一个缺陷条目）。
...
3. 发现缺陷记录到 `bug-log/auto-YYYY-MM-DD.md`。

### 4.3 缺陷分类与记录
**执行者**：QA 工程师

所有发现的缺陷（自动+探索）必须按 `skills/bug-triage-classification.md` 进行分类：
...
每次记录使用 `templates/bug-log-template.md`。
...
1. QA Agent 将 `manual-test-guide.md` 提交给用户，附一段摘要。
2. 用户按指南执行人工测试。
3. 用户将发现的缺陷（如有）反馈到 `bug-log/manual-YYYY-MM-DD.md`，使用 bug-log 模板。
...
- `bug-log/auto-YYYY-MM-DD.md`
- `bug-log/manual-YYYY-MM-DD.md`
- `test-results/regression-YYYY-MM-DD.log`