...
- `context/consistency-baseline.md`
- 知识图谱（`graphify-out/`，若不可用则以手动方式补充）

## 3. 强制规则
- `consistency-first.md`
- `api-compatibility.md`
- `reuse-before-build.md`
- `conflict-resolution.md`（设计冲突升级用）
- `knowledge/global/exception-handling.md`（如有）
...
2. **详细设计**：
   ...
   - 设计模式：显式声明用了项目中的哪个现有模式，引用 `consistency-baseline.md` 中的条目。
...
## 7. 阶段结束
- PM 更新 `iterations/sprint-status.md`，标记阶段 2 完成。